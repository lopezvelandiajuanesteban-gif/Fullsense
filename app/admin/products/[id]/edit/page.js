...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      if (error) {
        console.error(error);
        setMessage("No se pudo cargar el perfume.");
        setLoading(false);
        return;
      }
      ...
      setForm({
        ...
        brand: data.brand || "",
        price: data.price || "",
        category: data.category || "Hombre",
        type: data.type || "Árabe",
        size_ml: data.size_ml || "100",
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      console.error(error);
      setMessage("No se pudo actualizar el perfume.");
      setSaving(false);
      return;
    }

    setMessage("Perfume actualizado correctamente.");
